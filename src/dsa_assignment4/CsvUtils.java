...
public class CsvUtils
{
		...
		throws Exception
	{
		...
		try (Scanner from = new Scanner(fromPath)) {
			CsvFormatter csvFormatter = new CsvFormatter(from);
			CsvFormatter.RowComparator rowComparator = csvFormatter.new RowComparator(columnName);
			PriorityQueue<String[]> pQueue = new PriorityQueue<String[]>(rowComparator);
			String[] row;
			int tempNum = 0;
			while (from.hasNext()) {
				int currentRow = 0;
				while(currentRow < numRowLimit&&from.hasNext()) {
					currentRow++;
					row = csvFormatter.readRow(from);
					pQueue.add(row);
				}		
				...
				try (PrintWriter to = new PrintWriter(tmpPath.toFile())) {
					csvFormatter.writeHeader(to);
					int pQueueSize = pQueue.size();
					for (int i = 0; i < pQueueSize; i++) {
						csvFormatter.writeRow(to, pQueue.poll());
					}
				}
				tempNum = tempNum + 1;
			}
		...
		throws Exception
	{
		...
		try(Scanner from1 = new Scanner(file1Path);Scanner from2 = new Scanner(file2Path)){
			CsvFormatter csvFormatter1 = new CsvFormatter(from1);
			CsvFormatter csvFormatter2 = new CsvFormatter(from2);
			CsvFormatter.RowComparator rowComparator = csvFormatter1.new RowComparator(columnName);
			PriorityQueue<String[]> pqArray = new PriorityQueue<>(rowComparator);
			String[] row;
			while(from1.hasNext()) {
				row = csvFormatter1.readRow(from1);
				pqArray.add(row);
			}
			while(from2.hasNext()) {
				row = csvFormatter2.readRow(from2);
				pqArray.add(row);
			}
			try(PrintWriter to = new PrintWriter(outputPath.toFile())){
				csvFormatter1.writeHeader(to);
				while(!pqArray.isEmpty()) {
					csvFormatter1.writeRow(to, pqArray.poll());
				}
			}
		}
		return true;
		...
		throws Exception
	{
		...
		for(int i = 0;i<pathList.length-1;i++) {
			Path path1 = paths.getLast();
			paths.removeLast();
			Path path2 = paths.getLast();
			paths.removeLast();
			try(Scanner from1 = new Scanner(path1);Scanner from2 = new Scanner(path2)){
				CsvFormatter csvFormatter1 = new CsvFormatter(from1);
				CsvFormatter csvFormatter2 = new CsvFormatter(from2);
				CsvFormatter.RowComparator rowComparator = csvFormatter1.new RowComparator(columnName);
				PriorityQueue<String[]> pqArray = new PriorityQueue<>(rowComparator);
