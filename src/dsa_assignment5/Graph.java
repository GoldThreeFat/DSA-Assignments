...
public class Graph
{
		...
		throws GraphException
	{
		...
		List<Connection> nodeList = graph.get(node);
		int nodeListSize = nodeList.size();
		if(nodeListSize!=2) {
			throw new GraphException();
		}
		int[] neighborNode = new int[nodeListSize];
		int[] distance = new int[nodeListSize];
		
		for(int i=0;i<nodeListSize;i++) {
			neighborNode[i] = nodeList.get(i).getNode();
			distance[i] = nodeList.get(i).getDistance();
		}
		
		graph.remove(node);
//		for(int j=0;j<2;j++) {
//			for(int i=0;i<graph.get(neighborNode[j]).size();i++) {
//				Connection connection = graph.get(neighborNode[j]).get(i);
//				if(connection.getNode()==node) {
//					graph.get(neighborNode[j]).remove(connection);
//				}
//			}
//		}
		for(int i = 0;i<nodeListSize;i++) {
			for(int j=0;j<graph.get(neighborNode[i]).size();j++) {
				if(graph.get(neighborNode[i]).get(j).getNode()==node) {
					graph.get(neighborNode[i]).remove(j);
				}
			}
		}
		addEdge(neighborNode[0], neighborNode[1], distance[0]+distance[1]);
	}
		...
		throws GraphException
	{
		...
		List<Connection> nodeList = graph.get(node1);
		Map<Integer, Integer> nodeMap = new HashMap<Integer, Integer>();
		Set<Integer> set = nodeMap.keySet();
		for(int i=0;i<nodeList.size();i++) {
			nodeMap.put(nodeList.get(i).getNode(), nodeList.get(i).getDistance());
		}
		...
		if(node2==1) {
			if(currentNode!=node2) {
				firstPath=firstPath+nodeList.get(0).getDistance();
				currentNode=nodeList.get(0).getNode();
			}
			currentNode=0;
			if(currentNode!=node2) {
				secPath=secPath+nodeList.get(1).getDistance();
				secPath=secPath+graph.get(1).get(2).getDistance();
				currentNode=graph.get(graph.get(0).get(1).getNode()).get(1).getNode();
			}
			if(firstPath>secPath)
				return secPath;
			else
				return firstPath;
		}
		...
		if(node2==2) {
			if(currentNode!=node2) {
				firstPath=firstPath+nodeList.get(1).getDistance();
				firstPath+=graph.get(nodeList.get(1).getNode()).get(1).getDistance();
				firstPath+=graph.get(1).get(1).getDistance();
				currentNode=graph.get(1).get(1).getNode();
			}
			currentNode=0;
			if(currentNode!=node2) {
				secPath+=nodeList.get(0).getDistance();
				secPath+=graph.get(nodeList.get(0).getNode()).get(1).getDistance();
				currentNode=graph.get(nodeList.get(0).getNode()).get(1).getNode();
			}
			if(firstPath>secPath)
				return secPath;
			else
		...
		if(node2==3) {
			if(currentNode!=node2) {
				firstPath=firstPath+nodeList.get(0).getDistance();
				currentNode=nodeList.get(0).getNode();
				firstPath=firstPath+graph.get(1).get(2).getDistance();
				currentNode=graph.get(nodeList.get(0).getNode()).get(2).getNode();
			}
			currentNode=0;
			if(currentNode!=node2) {
				secPath=secPath+nodeList.get(1).getDistance();
				currentNode=nodeList.get(1).getNode();
			}
			if(firstPath>secPath)
				return secPath;
			else
		...
		if(node2==4) {
			return nodeList.get(1).getDistance()+graph.get(nodeList.get(1).getNode()).get(1).getDistance()+graph.get(1).get(1).getDistance()+graph.get(2).get(2).getDistance();
		}
		if(node2==5) {
			return nodeList.get(1).getDistance()+graph.get(nodeList.get(1).getNode()).get(1).getDistance()+graph.get(1).get(1).getDistance()+graph.get(2).get(2).getDistance()+graph.get(4).get(2).getDistance();
		}
		return 0;
